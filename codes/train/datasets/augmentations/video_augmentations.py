...
class StrongTransforms(object):
    ...
    def __init__(self,
        ...
        # for training
        train_transforms = [video_transforms.RandomResizedCrop(crop, scale=(min_area, 1.))]
        if apply_hf>0:
...
class RawAug(object):
    ...
    def __init__(self,
        ...
        """ we perform min max normalization here: [-1, 1] """
        ...
        def _prepare_transformations(transforms):
            if totensor:
                transforms += [volume_transforms.ClipToTensor()]
                if normalize:
                    transforms += [video_transforms.MinMaxNormalize(min_max=min_max)]
            transform = video_transforms.Compose(transforms)
            return transform
