...
# from engine.video.video_svm import linear_svm as worker
...
def get_args():
    ...
    ## pretrained model
    parser.add_argument("--weight_path_vid", default="/path/to/vid", help="checkpoint_path for backbone restoration.")
    parser.add_argument("--weight_path_aud", default="/path/to/aud", help="checkpoint_path for backbone restoration.")

    ## dataset and config
    parser.add_argument("--db", default="kinetics_sound", help="target db", choices=['kinetics_sound'])  
    parser.add_argument('-c', '--config-file', type=str, help="pass config file name w/o extension", default="aud_vid_4s_112_v1")
...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    from engine.audio_video.audio_video_svm import linear_svm as worker
    ...
    vidpath = args.weight_path_vid
    audpath = args.weight_path_aud
    
    state = {}
    if os.path.isfile(vidpath):
        if args.gpu is None:
            state['video'] = torch.load(vidpath)
        else:
            state['video'] = torch.load(vidpath, map_location='cuda:{}'.format(args.gpu))
    else:
        raise FileNotFoundError (f'weight is not found at {vidpath}')

    if os.path.isfile(audpath):
        if args.gpu is None:
            state['audio'] = torch.load(audpath)
        else:
            state['audio'] = torch.load(audpath, map_location='cuda:{}'.format(args.gpu))
    else:
        raise FileNotFoundError (f'weight is not found at {audpath}')
