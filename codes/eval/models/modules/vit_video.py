...
class VideoViT(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, t, w, h):
        ...
        if t0 / Nt ==1:
            # not interpolating the time axis
            patch_pos_embed = nn.functional.interpolate(
                patch_pos_embed.reshape(int(Nt),
                                        int(math.sqrt(Ns)), 
                                        int(math.sqrt(Ns)), 
                                        dim).permute(3, 0, 1, 2),
                scale_factor=(w0 / math.sqrt(Ns), h0 / math.sqrt(Ns)),
                mode='bicubic'
            )
            
            assert int(t0) == patch_pos_embed.shape[-3] and int(w0) == patch_pos_embed.shape[-2] and int(h0) == patch_pos_embed.shape[-1] 
            patch_pos_embed = patch_pos_embed.permute(1, 2, 3, 0).view(1, -1, dim)
...
if __name__=='__main__':
    ...
    model = VideoViT(frame_size = (3, 112, 112),
                            num_frames = 32,
                            patch_spatial = (16, 16),
                            patch_temporal = 4,
                            vit_config = 'tiny',
    ...
    frames0 = torch.randn(2, 3, 32, 224, 224)
    feat0 = model(frames0, feat_op='pool')
