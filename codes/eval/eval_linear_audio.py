...
def get_args():
    ...
    ## dataset and config
    parser.add_argument("--db", default="esc50", help="target db")
    parser.add_argument('-c', '--config-file', type=str, help="pass config file name w/o extension", default="config_v1")
...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    if args.db in ['fsd50']: # mAP
        from engine.audio.audio_linear_map import finetune as worker
        worker(args, cfg, state, ngpus_per_node, logger=logger, tb_writter=tb_writter, wandb_writter=wandb_writter)

    else: # top1 acc
        from engine.audio.audio_linear import finetune as worker
        worker(args, cfg, state, ngpus_per_node, logger=logger, tb_writter=tb_writter, wandb_writter=wandb_writter)
