...
def get_args():
    ...
    ## dataset and config
    parser.add_argument("--db", default="charades", help="target db", choices=['ucf101', 'hmdb51', 'kinetics400', 'ssv2', 'charades', 'kinetics_sound'])  
    parser.add_argument('-c', '--config-file', type=str, help="pass config file name w/o extension", default="fold1_32f_112_16x4_cj_hf_base_pool_3crop")
...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    if os.path.isfile(path):
        if args.gpu is None:
            ...
            if args.cf.startswith('online'):
                state = torch.load(path, map_location='cuda:{}'.format(args.gpu))['model']
            else:
