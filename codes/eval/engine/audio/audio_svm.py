...
from engine import set_grad, Feature_Bank
...
from models import AudioViT, SVMWrapper
...
def linear_svm(args, cfg, backbone_state_dict, logger, tb_writter, wandb_writter):
    ...
    # get model
    model=SVMWrapper(backbone=AudioViT(**cfg['model']['backbone']), 
    ...
    if args.debug:
        train_dataset = FetchSubset(train_dataset, 128)
        val_dataset = FetchSubset(val_dataset, 16)
    ...
    if cfg['dataset']['name'] in ['esc50', 'dcase', 'kinetics_sound']:
            _compute = _compute_acc
    # elif cfg['dataset']['name'] in ['fsd50']:
    #     _compute = _compute_map
    else:
        raise NotImplementedError(f'unknown eval metric for {cfg["dataset"]["name"]}')
...
def _compute_acc(cost, cfg, logger,
             train_features, train_labels, train_indexs,
             val_features, val_labels, val_indexs,
