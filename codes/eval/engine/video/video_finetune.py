...
import math
import sys
from engine import AverageMeter, ProgressMeter, accuracy, warmup_cosine_scheduler, warmup_multistep_scheduler, get_params_groups, Classifier
...
def finetune(args, cfg, backbone_state_dict, ngpus_per_node, logger, tb_writter, wandb_writter):
    ...
    test_batch_size = max(cfg['dataset']['batch_size'] // (3*cfg['dataset']['test']['clips_per_video']), 2)
    ...
    # elif cfg['hyperparams']['weight_decay']['name'] == 'fixed':
    #     iters = cfg['hyperparams']['num_epochs'] * len(train_loader)
    #     wd_scheduler = np.ones(iters) * cfg['hyperparams']['weight_decay']['base']
    # else:
    #     raise NotImplementedError()
