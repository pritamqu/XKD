...
# from engine.video.video_finetune import finetune as worker
...
def get_args():
    ...
    ## dataset and config
    parser.add_argument("--db", default="fsd50", help="target db", choices=['esc50', 'dcase', 'fsd50', 'kinetics_sound', 'vggsound'])
    parser.add_argument('-c', '--config-file', type=str, help="pass config file name w/o extension", default="config_v3")
...
def main_worker(gpu, ngpus_per_node, args, cfg):
    ...
    if args.db in ['fsd50']: # mAP
        from engine.audio.audio_finetune_map import finetune as worker
        worker(args, cfg, state, ngpus_per_node, logger=logger, tb_writter=tb_writter, wandb_writter=wandb_writter)
        
    else: # ESC -> Acc.
        from engine.audio.audio_finetune import finetune as worker
        worker(args, cfg, state, ngpus_per_node, logger=logger, tb_writter=tb_writter, wandb_writter=wandb_writter)
